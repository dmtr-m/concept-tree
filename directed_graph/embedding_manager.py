...
def reorder_ngram(phrase):
    ...
    words_with_importance = [(token.text, get_word_importance(token), token.dep_) for token in doc]
...
def get_embedding(words: list[str]) -> NDArray[np.float64]:
    ...
    try:
        if len(words) == 1:
            return embeddings_dict[words[0]]
        else:
            sorted_words = reorder_ngram(" ".join(words))
            embeddings = [embeddings_dict[word] for word in sorted_words]
